            ...
            if(response.deductible_panel && response.deductible_panel.length > 10)
            {
                ...
                if(response.installment_panel && response.installment_panel.length > 10)
                {
                    ...
                    for(var i = 0; i < installment_panel.length; i++) 
                    {
                        ...
                        $('#installmentPaneldetail tbody').prepend('<tr id="iidinstallmentdetail'+obj.id+'" data-name="installmentdetailvalue[]"><td data-name="'+obj.installment_date+'">'+strdate+'</td><td data-name="'+obj.percentage+'">{{ number_format('+parseFloat(obj.percentage)+', 2, '.', ',') }}</td><td data-name="'+curr_amount+'">{{ number_format('+parseFloat(curr_amount)+', 2, '.', ',') }}</td><td></td></tr>')
